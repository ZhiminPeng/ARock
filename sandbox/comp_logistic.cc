...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  int num_tests = (int) (log2(total_num_threads)) + 1;
  vector< vector<double> > result(num_tests, vector<double>(5));
...
# pragma omp parallel num_threads(thread_count) shared(A, b, x, Atx, para)
    ...
    result[test_count][0] = thread_count;
    result[test_count][1] = end - start;
    result[test_count][2] = l1_objective(A, b, x, Atx, para);
...
# pragma omp parallel num_threads(thread_count) shared(A, b, x, Atx, para)
    {
      syn_l1_logistic(A, b, x, Atx, para);
    }
    double end = omp_get_wtime();
    
    result[test_count][3] = end - start;
    result[test_count][4] = l1_objective(A, b, x, Atx, para);
  ...
  cout<<"---------------------------------------------"<<endl;
  cout<<setw(15)<<"# cores";
  cout<<setw(15)<<"asyn time(s)";
  cout<<setw(15)<<"asyn obj";
  cout<<setw(15)<<"syn time(s)";
  cout<<setw(15)<<"syn obj";
  cout<<endl;
  for(int i=0;i<num_tests;i++)
  {
    cout<<setw(15)<<setprecision(2)<<result[i][0];
    cout<<setw(15)<<setprecision(2)<<scientific<<result[i][1];
    cout<<setw(15)<<setprecision(2)<<result[i][2];
    cout<<setw(15)<<setprecision(2)<<scientific<<result[i][3];
    cout<<setw(15)<<setprecision(2)<<result[i][4];
    cout<<endl;
  }
  cout<<"---------------------------------------------"<<endl;
