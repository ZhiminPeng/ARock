...
int main(int argc, char *argv[])
{
  ...
  num_tests = (int) (log2(total_num_threads)) + 1;
...
# pragma omp parallel num_threads(thread_count) shared(A, b, x, para)
    ...
    result[test_count][2] = calculate_residual(A, b, x);
    ++test_count;
  }
...
# pragma omp parallel num_threads(thread_count) shared(A, b, x, para)
    {
      // balanced_jacobi(A, b, x, para);
      syn_jacobi(A, b, x, para);
    }
    double end = omp_get_wtime();
    result[test_count][3] = end - start;
    result[test_count][4] = calculate_residual(A, b, x);
    test_count++;
  }
  ...
  cout<<setw(15)<<"asyn ||Ax -b||";
  cout<<setw(15)<<"syn time(s)";
  cout<<setw(15)<<"syn ||Ax -b||";
  cout<<endl;
